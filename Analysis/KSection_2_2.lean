...
theorem KNat.le_total (n m : KNat) : n ≤ m ∨ m ≤ n := by
  induction' m with k hk
  apply Or.inr
  rw [zero_eq]
  use n
...
theorem KNat.lt_TFAE (n m : KNat) :
  ...
  tfae_have 5 → 2 := by
    ...
    use predp
  tfae_have 5 → 6 := by
    rintro ⟨p,⟨h1,rfl⟩⟩ ⟨ℓ,hℓ⟩
    rcases p with (_|predp)
    rw [zero_eq] at h1
    apply h1
    rfl
    rw [add_assoc,succ_add] at hℓ
    have h1 := ne_add_succ n (predp + ℓ)
    exact h1 hℓ
  tfae_have 6 → 4 := by
    have h1 : (m ≤ n) ∨ (n ≤ m) := le_total m n
    rcases h1 with (h1|h1)
    intro h0
    apply False.elim (h0 h1)
    intro h2
    apply And.intro h1
    intro h3
    apply h2
  ...
  tfae_have 7 → 4 := by
    ...
    use 0
    rw [zero_eq,add_zero,add_zero]
    have h1 := ne_add_succ n predℓ
    exact h1 h0
  tfae_have 4 → 7 := by
    rintro ⟨h0,h1⟩
    apply And.intro h0
    contrapose! h1
    rcases h0 with ⟨ℓ,rfl⟩
    rcases ℓ with (_|predℓ)
    rw [zero_eq,add_zero]
    rcases h1 with ⟨p,hp⟩
    apply False.elim
    rw [add_assoc,succ_add] at hp
    have h1 := ne_add_succ n (predℓ +p)
    exact h1 hp    
  tfae_finish
...
lemma KNat.le_of_lt {n m:KNat} (hnm: n < m) : n ≤ m := by
  rcases hnm with ⟨ℓ,rfl⟩
  use ℓ♯  
...
theorem KNat.ge_antisymm {a b:KNat} (hab: a ≥ b) (hba: b ≥ a) : a = b := by
  ...
  rw [add_zero]
  apply False.elim
  rcases hba with ⟨ℓ,hℓ⟩
  rw [add_assoc,succ_add] at hℓ
  have h1 := ne_add_succ b (predp + ℓ)
  exact h1 hℓ 
